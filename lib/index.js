...
module.exports = () => {
  ...
  const get = (path, handler) => {
    ...
    const dynamicPaths = path.split('/').filter(x => x !== '').filter(x => /:/.test(x)).map(x => x.replace(':', ''));
    const pathParams = path.replace(new RegExp(':', 'g'), '').split('/').filter(x => x !== '');

    dynamicPaths.forEach(x => dynamicRoutePaths.add(x));
